...
function extendsReducer(reducerHandler,routeConfig,store){
    ...
    // updateStateIfReturnNewState(reducer,reducerHandler.$update,reducerHandler.$state)
    // read formdata
    reducerHandler.$valueOf=()=>{
        return reducerHandler.__formData||{};
    }
    /**
     *
     * @param value value
     */
    reducerHandler.$valueAlter=(field,value)=>{
        reducerHandler.__formData[field]=value;
    }

    // 将表单数据连接到store中去，默认存在formData中
    reducerHandler.$link             =(storeKey="formData")=>  {
        return {
            onFieldsChange(prop, obj){
                let fieldName = Object.keys(obj)[0];
                if(!reducerHandler.__formData){
                    reducerHandler.__formData={};
                }
                if(obj[fieldName]){

                    reducerHandler.__formData[fieldName]=obj[fieldName]['value'];
                }
     ...
     * @param values
     * @param form
     * @forceUpdate {boolean} 总是使用初始值
     */
    reducerHandler.$initalFormData=(values,form,forceUpdate,onInitCb)=>{

        if(!reducerHandler.__formData){
            reducerHandler.__formData={}
        }
        let formData = reducerHandler.$valueOf();
        for(let field in values){
            if(forceUpdate || !formData[field]){
                console.log(field,values[field])
                formData[field]=values[field];
                reducerHandler.__formData[field]=values[field];
            }
        }
