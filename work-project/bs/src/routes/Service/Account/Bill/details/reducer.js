...
export const getServieList=(id)=>{
  axios.post('openApi/serviceBill/getServieList',{order_no:id}).then((res)=>{
    ...
    itemArr.map((item)=>{
      ...
      if(item.buy_count&&item.product_market_price)sum+=item.buy_count*item.product_market_price;
      arr.push(obj);
    })
    if(itemArr.length){
      itemArr[0].sum=sum;
...
export const getList=(params)=>{
  axios.post("openApi/serviceBill/getList",params).then((res)=>{
    handler.$update(exports.state.set('list',Immutable.List(res.data)).set('loading',false))
  })
}
export const getDetail=(params)=>{
  axios.post('openApi/serviceBill/getDetail',params).then((res)=>{
    handler.$update(exports.state.set('dataSource',Immutable.List(res.data)))
  })
}//账单明细
