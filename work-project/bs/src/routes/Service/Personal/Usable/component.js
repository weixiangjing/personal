...
import {Menu, Row, Col, Form, Input, Select, Button, Icon, AutoComplete, Progress, Alert, Collapse} from "antd";
...
export default React.createClass({
  ...
  render(props, state){
    ...
    return (
      <div>
        <AutoForm ref={(form)=>this.form=form} onSubmit={()=>cardTable.reload(this.setHandlerValue())}/>
        <CardTable
          ref={(t)=>cardTable=t}
          url="openApi/purchasedService/getSurplusList"
          ...
          renderContent={(list)=>{
             ...
             {list.map((item,index)=>{
                return(
                <Panel header={<span className="main-title"><i className={`font-md text-shade margin-h fa ${item.unit_type==1?"fa-user":item.unit_type==2?"fa-home":item.unit_type==3?"fa-mobile":"fa-odnoklassniki-square"}`}></i><span>{item.unit_id}</span></span>} key={`sub${index}`}>
  ...
  render(){
    ...
    return (
      <Form layout={'inline'} onSubmit={(e) => {
            ...
            <FormItem label="计费单元：" className="unitid">
