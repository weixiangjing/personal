            ...
            getComponent (nextState, cb) {
                ...
                require.ensure([], (require) => {
                    ...
                    if(routeConfig.storeKey && routeConfig.reducer){
                        const Reducer = require('../routes/'+routeConfig.reducer).default;
...
function initReducer(store,routeConfig,Reducer) {
    ...
    reducer.initialize(store,routeConfig.storeKey);
    function createReducer(state = reducer.initialState(),action) {
        ...
        const handler = reducer.performUpdate;
        return handler ? reducer.performUpdate(state, action) : state
    }
