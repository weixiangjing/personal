...
import { Tree, Input ,Button,notification,Icon,Menu, Dropdown,Row,Col,Modal,Spin,Table} from 'antd';
...
/*export default React.createClass({
  ...
  onDrop(info) {
    ...
    if (info.dropToGap) {
      ...
      //ar.splice(i+1, 0, dragObj);//拖到指定元素上方
    } else {
  ...
  render(props,state) {
    ...
    const loop = data => data.map((item) => {
      ...
      if (item.res_childs.length>0) {
        return (
          <TreeNode key={resId} title={
            <div>{item.res_type=='Action'?<i className="fa fa-legal"></i>:''}<span onClick={()=>this.onSelect(resId,item)} className="lg-icon">{res_name}</span>&nbsp;
              {this.state.keySelect==resId?
              <span>
                <Icon type="edit" onClick={this.onEdit} className="lg-icon"/>&nbsp;
                <Icon type="minus" onClick={()=>this.onMinus(resId,MINUS,item.res_name)} className="lg-icon"/>&nbsp;
                <Dropdown className="lg-icon" overlay={this.menuHtml(resId)}><i className="ant-dropdown-link"><Icon type="plus"/></i></Dropdown>
              </span>
              :''
              }
            </div>
      ...
      return <TreeNode key={resId} title={<div>{item.res_type=='Action'?<i className="fa fa-legal"></i>:''}
...
export default React.createClass({
  ...
  handleCreate(){
    ...
    form.validateFields((err, values) => {
      ...
      if(text||index){
        reducer.EditResource(values,index)
      }else {
        reducer.ResourceNew(values);
      }
      form.resetFields();
  ...
  SwicthStatus(text,index){
    reducer.Swicth(text,index);
    this.setState({
      visible: false,
    })
  },
  render(props,state){
    ...
    const columns=[{
      ...
      render: (text, record, index) => {
        ...
        return (
          <span>
            <a onClick={()=>this.showModal(text,index)}>编辑</a>

            <span className="ant-divider"/>
            <a onClick={()=>this.SwicthStatus(text,index)}>{status==1?'停用':'启用'}</a>
          </span>
        );
      },
    }];
    /*const pagination = {
      ...
      onShowSizeChange: (current, pageSize) => {
        const params={pageSize:pageSize,pageNum:current};
        this.setState({loading:true,page:params,current:current})
        props.getUserList(this.callback,params).then(()=>{
          this.setState({loading:false})
        })
      },
      onChange: (current) => {
        const params={pageNum:current,pageSize:10};
        this.setState({loading:true,page:params,current:current})
        props.getUserList(this.callback,params).then(()=>{
          this.setState({loading:false})
        });
    ...
    return (<div>
      <Button type="primary" onClick={()=>this.showModal(null,null)} style={{marginBottom:15,float:'right'}}>新建用户</Button>
      <RegistrationForm
      ...
      />
      <Table
        dataSource={state.get('list').toJS()}
        columns={columns}
