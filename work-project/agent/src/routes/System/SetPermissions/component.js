...
import { Tree, Input ,Button,notification,Icon,Menu, Dropdown,Row,Col,Modal} from 'antd';
...
export default React.createClass({
  ...
  onMinus(key,str,name){
    ...
    Modal.confirm({
      ...
      cancelText: '取消',
      onOk() {
        if(key=='new'){me.props.getResourceList(me.callback)}
        else {
  ...
  render(props,state) {
    ...
    const loop = data => data.map((item) => {
      ...
      if (item.res_childs.length>0) {
        return (
          <TreeNode key={resId} title={
            <div><span onClick={()=>this.onSelect(resId,item)} className="lg-icon">{res_name}</span>&nbsp;
      ...
      return <TreeNode key={resId} title={<div>
