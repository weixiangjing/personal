...
export default React.createClass({
  ...
  componentWillMount(){
    this.props.getRoleList({operator_id:String(User.userid),status:'1'});
  },
  saveFormRef(form){
    this.form = form;
  },
  ...
  callbake(data){
    ...
    const Arr=[];
    const userM=[];
    const loopUserMenus=data=>data.map((item)=>{
      if(item.res_childs){loopUserMenus(item.res_childs)}
      userM.push(item.res_id)
    })
    loopUserMenus(User.originMenus);
    ...
    /*if(User.username.toLocaleLowerCase()=="admin"){
      const falseD=data=>data.map((item)=>{
        if(item.res_childs){falseD(item.res_childs)}
