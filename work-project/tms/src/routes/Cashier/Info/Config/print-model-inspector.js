...
import {handler,deleteControl} from "./reducer";
...
export default class InspectorForm extends React.Component{
    ...
    onValueChange(value,item){
        let id = handler.$state("printInspector").get("id");
        let controls =handler.$state("printControls")
        let findIndex =controls.findIndex(item=>item.get("id")==id);
        handler.$update(
            handler.$state()
            .updateIn(['printControls',findIndex,'props',item],()=>value)
            .updateIn(['printInspector','props',item],()=>value));
    }
    
    render(){
        
        let control =handler.$state("printInspector");
        ...
        return ( <div>
            {
                Object.keys(_props).map((item,index)=>{
                    ...
                    return <div className="print-inspector-item" key={control.get("id")+index}>
                       ...
                       <DynamicControl item={item} value={_props[item]} onChange={(v)=>this.onValueChange(v,item)}/>
                        
                    </div>
                })
            }
            <Button style={{marginLeft:70,marginTop:24}} onClick={()=>deleteControl()} type="danger">删除</Button>
        </div>)
    }
}
