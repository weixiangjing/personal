...
import {SYSTEM_OPERATIONLOG_DOWNLOAD,SYSTEM_OPERATIONLOG_DETAIL} from '../../../config/auth_func'
    ...
    for(let k in value){if(value[k]&&k!='start_date'&&typeof(value[k])!="number"){value[k]=value[k].trim();}}
        ...
        const columns=[{
          ...
          //{title: '计费单元标识', dataIndex: 'log_tag'},
          {
            ...
            title : '操作',
            key   : 'action',
            render: (text, record, index)=>(<Auth to={SYSTEM_OPERATIONLOG_DETAIL}><a onClick={()=>this.showModal(text)}>查看</a></Auth>)
        }];
        return (<div>
            ...
            <Table
                ...
                ref={t=>creatTable=t}
                extra={
            <Auth to={SYSTEM_OPERATIONLOG_DOWNLOAD}>
              <Button onClick={()=>axios.post('serviceLog/getLog',{...this.getValue(),"export":"1"}).then(()=>showTaskModal())}>导出数据</Button>
            </Auth>
            }
            />
