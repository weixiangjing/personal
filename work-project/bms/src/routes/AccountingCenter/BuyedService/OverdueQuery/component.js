...
export default React.createClass({
  ...
  render(props,state){
    ...
    const columns=[
      ...
      {
        title: '操作',
        render:(text, record, index)=>(<Auth to={ACCENTER_BUYED_OVERDUE_LINK}><Link to={`accenter/license/query/detail?id=${record.license_code}&type=${record.unit_type}&unit=${record.unit_id}`}>最近许可</Link></Auth>)
      }
    ];
    return (
      <div>
