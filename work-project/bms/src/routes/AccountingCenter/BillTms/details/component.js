...
import {ACCENTER_BILLTMS_DETAILS_MODIFYPRICE,ACCENTER_BILLTMS_DETAILS_LINK} from '../../../../config/auth_func'
...
export default React.createClass({
  ...
  handleCreate(){
    ...
    form.validateFields((err, values) => {
      ...
      values.manual_adjust_note=values.manual_adjust_note.trim();
      updateBill(values).then(()=>{
        getList(params)
        getServieList(id);
        billTable.update({order_no:id})
        this.setState({
          visible: false
        })
      })
    ...
    const columns =[
      ...
      },
      {
        ...
        render:(text, record, index)=>{
          return(
          <span className="product-tb">【{record.service_name||record.combo_name}】{record.product_name}</span>
        )}
      },
      {
        title: '计费单元',
      ...
      {
        title: '操作',
        render:(text, record, index)=>(<Auth to={ACCENTER_BILLTMS_DETAILS_LINK}><Link to={`accenter/license/query?unit_type=${record.unit_type}&unit_id=${record.unit_id}&product_code=${record.product_code}&service_code=${record.service_code}`}>查看许可</Link></Auth>)
      }
    ]
        ...
        {!state.get('loading')?(
          <div>
            <Card title={<span><b className="title_span">账单号：{id}</b><span className={`${_status==1?"color_1":_status==2?"color_2":"color_3"}`}>{_status==1?"待付款":_status==2?"已付款":"已关闭"}</span></span>} className="card_1"
                  extra={_status==2?<span>完成支付时间：{objItem.pay_time}</span>:_status==1?<Auth to={ACCENTER_BILLTMS_DETAILS_MODIFYPRICE}><Button onClick={()=>this.onShow(objItem)}>手工批价</Button></Auth>:<span>创建时间：{objItem.create_time}</span>}
            >
              <Row>
                <Col sm={16}>
