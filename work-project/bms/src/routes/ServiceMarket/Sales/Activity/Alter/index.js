...
import {Modal, Tabs,Popover,Icon, Input, Form, Radio, Button, DatePicker, Row, Col, Alert} from "antd";
    ...
    getRulePlaceholder(rule){
        ...
        if(Array.isArray(params))
            return params.map(item=>{
                this.ruleParams.push(item.type);
        ...
        return (
            <Form>
                ...
                <ControlItem formItemLayout={xLayout} name={'rule_id'} decorator={getFieldDecorator}
                    ...
                    <SaleRulesSelect onOptionsLoaded={(options,value)=>{
                ...
                <Form.Item {...xLayout} label="条件参数值">
                    {
                        getFieldDecorator("copy_of_input_param",{
                            rules: [{validator:(rule,value,cb)=>{
                                if(!value)cb("条件参数值必填");
                             
                                let valueArr= value.split(",").filter(item=>item);
                                for(let i=0;i<valueArr.length;i++) {
                                    let item=valueArr[i];
                                    if(this.ruleParams[i]==="number"&&isNaN(item)) {
                                        cb(`第${i+1}个条件参数值应该为数字类型`);
                                        return true;
                                    } else if(this.ruleParams[i]==="date"&&! /^\d{4}-\d{2}-\d{2}$/.test(item.trim())) {
                                        cb(`第${i+1}个条件参数值应该为日期类型（yyyy-mm-dd）例如：2012-09-24`);
                                        return true;
                                    }
                                }
                                cb()
                    ...
                    <Popover overlayStyle={{width:500,color:"#989898"}} content={<div>
                        {
                            rulePlaceholder && rulePlaceholder.split("@@@").map((item,index)=>(<div>条件{index+1 }: {item}</div>))
                        }
                        <div>多个条件使用英文逗号分隔</div>
                    </div>}>
                    <Icon className="margin-left font-md text-shade hover-pointer" type="question-circle-o"/>
                    </Popover>
                </Form.Item>

                <ControlItem required extra={"金额为正值表示优惠，负值表示上涨，单位 ：元"} formItemLayout={xLayout}
                             name={'discount_adjust_amount'}
                             decorator={getFieldDecorator} label={"优惠金额"}>
                    <Input style={{width: 100}} type={"number"}/>
                </ControlItem>
