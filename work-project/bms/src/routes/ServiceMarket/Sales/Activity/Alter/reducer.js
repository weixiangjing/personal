...
export function echoActivity(send,form1){
    getActivity(send).then(({data})=>{
        ...
        item.discount_budget_amount=decimal(item.discount_budget_amount).div(100).valueOf()
        item.discount_adjust_amount=decimal(item.discount_adjust_amount).div(100).valueOf()
                ...
                if(rule.length !=rule_values.length){
                    return form2.setFields({
                        copy_of_input_param:{
                            ...
                            errors:[new Error("缺少条件参数值，参数值与限定条件不匹配")]
                        }
