...
export function postProd(form) {
    form.validateFields((error, value) => {
        ...
        value.product_market_price=decimal(value.product_market_price).mul(100).valueOf();
        value.product_cost_price=decimal(value.product_cost_price).mul( 100).valueOf()
...
export function deleteProduct(send){
    return alterProduct(send).then(()=>showModal(false))
}