...
import {showModal, handler, handleProd, postProd, toggleStatus,deleteProduct} from "./reducer";
        ...
        const columns=[{
            ...
            title: "状态", className: "corner-mark", render: (a, col)=> {
                return <span className={billing_class[col.product_status]}><i
                    className="font-lg">.</i>{col.product_status==1 ? "发布" : "取消发布"}</span>
            }
        }, {
            ...
            title: "操作", render: (a, col)=> {
                let status=col.product_status==1 ? "取消发布" : "发布";
                return (<span>
...
class AlterForm extends React.Component {
    ...
    render() {
        ...
        return (
            <Modal
                footer={
                    <div>   <Button onClick={()=>showModal(false)}>取消</Button>
                        <Button onClick={()=> postProd(this.props.form)} type={"primary"}>确认</Button>
                     
                        {
                           product_code && <Popconfirm title="确认要删除该产品吗?" onConfirm={()=>deleteProduct({is_delete:2,product_code}).then(()=>Table.getTableInstance().reload())}>
                                <Button className={"pull-left"} type={"danger"}>删除服务产品</Button>
                            </Popconfirm>
                        }
                    </div>
                }
