...
let unit_type = 1;
let license_table;
const success_update_types=['','更换计费单元','冻结','解冻']
class AlterForm extends React.Component {
    render() {
        ...
        let license_codes =getFieldValue("product_codes");
            ...
            <Form >
                ...
                
                {
                    step==1&&<div className="step-2" >
                        ...
                        {getFieldDecorator('product_codes')(
                            <SelectionTableFormItem params={Object.assign({unit_type},getFieldsValue())} type={values.update_type}/>
                        )}
...
const new_license_class=['text-success','text-danger'];
const new_license_state=["未冻结","已冻结"]
class SelectionTableFormItem extends React.Component {
    render() {
        let {onChange,type,params} = this.props;
        let columns                                             = [
            {
                title    : "产品分组",
            ...
            {
                title    : "最后失效时间",
                dataIndex: "current_service_exp_date"
            },
            {
                title : "冻结状态",
                className:"corner-mark",
                render: (a, col) => <span className={new_license_class[col.is_frozen]}><i className="font-lg">.</i>{new_license_state[col.is_frozen]}</span>
            },
            {
                title : "计量总量",
            ...
            <div>
                <Table
                    ...
                    onSelect   : (record, selected, selectedRows) => {
                        ...
                        onChange(selectedRows.map(item=>item.product_code).join(","))
                    },
                    onSelectAll: (selected, selectedRows, changeRows) => {
                       onChange(selectedRows.filter(item=>{
                           if(type==2){
                               return item.is_frozen==0;
                           }else if(type==3){
                               return item.is_frozen==1;
                           }
                           
                       }).map(item=>item.product_code).join(","));
                    },getCheckboxProps:(records,a)=>{
                   
                        let props= {disabled:true}
                        if(type==2 && (records.service_type==2||records.is_frozen==1)){
                           return  props
                        }
                        if(type==3 && (records.service_type==2||records.is_frozen==0)){
                            return  props
                    ...
                    license_table = t;
                }} rowKey={"license_code"} url={'serviceLicense/getServiceOfLicense'} columns={columns} />
            </div>
        )
    }
}