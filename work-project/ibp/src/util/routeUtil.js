...
export function loadRoutes(routeConfigList,store) {
    ...
    routeConfigList.forEach(function (routeConfig) {
        ...
        const route = {
            ...
            breadcrumbName:routeConfig.title || routeConfig.name,
...
browserHistory.listen((nextLocation) => {
    if(['PUSH','REPLACE'].indexOf(nextLocation.action) !== -1 && nextLocation.pathname !== currentLocation.pathname){
