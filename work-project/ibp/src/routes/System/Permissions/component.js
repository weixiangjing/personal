...
export default React.createClass({
  ...
  componentWillMount(){
    this.props.getRoleList({operator_id:User.get('user_id'),status:"1"});
  },
  saveFormRef(form){
    this.form = form;
  },
  ...
  callbake(data){
    ...
    const loop=data=>data.map((item) => {
      ...
      if(item.checked==1){roleArr.push(String(item.res_id))}
      if(item.checked==0)Arr.push(String(item.parent_id));
      item.isdisabled=true;//禁用checkedbox
      unique(Arr).map((key)=>{
    ...
    loop(treeList);
    const p_arr=[];
    for(let k=0;k<userM.length;k++){
      const loop_user=(data)=>data.map((item) => {
        if(item.res_childs){loop_user(item.res_childs)}
        if(userM[k]==item.res_id)item.isdisabled=false;
      });
      loop_user(treeList);
    }
    const loop_p_arr=(data)=>data.map((item) => {
      if(item.res_childs){loop_p_arr(item.res_childs)}
      if(item.isdisabled)p_arr.push(item.parent_id);
      unique(p_arr).map((key)=>{if(key==item.res_id){p_arr.push(item.parent_id);}})
    });
    loop_p_arr(treeList);
    const loop_p=(data)=>data.map((item) => {
      if(item.res_childs){loop_p(item.res_childs)}
      unique(p_arr).map((key)=>{
        if(key==item.res_id&&item.res_childs.length){item.isdisabled=true;}
      })
    });
    loop_p(treeList);//开启权限范围内的checkedbox
  ...
  handleCreate(){
    ...
    const role_params={operator_id:User.get('user_id')};
  ...
  onChangeStatus(e){
    ...
    const params={operator_id:User.get('user_id')};
    if(value)params.status=value;
    this.setState({statusValue:value});
    this.props.getRoleList(params);
  },
render(props,state){
    const List=props.get('list').toJS();
  return (
    <div className="permissions">
      ...
      {!props.get('load')&&<p className="total-top">共搜索到{List.length}个结果</p>}
      <Spin spinning={props.get('load')}>
        <Row gutter={16}>
          {!props.get('load')?
            List.map((obj,i) => {
