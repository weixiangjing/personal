...
const SetForm = Form.create({
  onFieldsChange:(props, fields)=>{
    let data=props.SelectData;
    for(let key in data){
      for(let k in fields){
        if(key==k&&fields[k].value&&data[key]!=fields[k].value){
      ...
      <div>
        ...
        <Form layout={'horizontal'}>
          ...
          {getFieldDecorator('res_name',{
            rules: [{required: true, message: '请输入资源名称'},{whitespace:true,message: '资源名称不能为空格'}],
          ...
          {getFieldDecorator('status',{
            ...
            initialValue:SelectData.status||1
          })(
            <Radio.Group>
          ...
          >
            {getFieldDecorator('res_url',{
              rules: [{required: true, message: '请输入资源地址'},{whitespace:true,message: '资源地址不能为空格'}],
