...
export default React.createClass({
  ...
  onSelect( selectedKeys,info){
    const {isFormChange,keySelect}=this.state;
    ...
    if(setForm){
      setForm.validateFields((err, values) => {
        if (err) {isChecked=false;return;}
        if(keySelect!='new')setForm.resetFields();
      })
    }
  ...
  handleCreate(){
    ...
    if(setForm){
      ...
      loop(gData,-1);
      const prams={operator_id:User.get('user_id'), res_list:gData};
      this.setState({loading:true})
      this.props.ResourceNew(prams).then(()=>{
        this.props.getResourceList(this.callback)
        this.setState({
